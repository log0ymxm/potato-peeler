...
public class TextClassifier
{
	...
	private final String delimitersStringToWordVector = "\\s.,:'\\\"()?!";
	...
	// constructor
	//
	TextClassifier(String[] inputText, String[] inputClasses,
			FastVector attributeInfo, Attribute textAttribute,
	...
	public static StringBuffer checkCases(Instances theseInstances,
			Classifier thisClassifier, Attribute thisClassAttribute,
			String[] texts, String testType, int startIx)
	{
		...
		try
		{
			...
			for (int i = startIx; i < theseInstances.numInstances(); i++)
			{
				...
				SparseInstance sparseInst = new SparseInstance(
						theseInstances.instance(i));
				sparseInst.setDataset(theseInstances);

				result.append("\nTesting: '" + texts[i - startIx] + "'\n");
				// result.append("SparseInst: " + sparseInst + "\n");

				double correctValue = sparseInst.classValue();
				double predictedValue = thisClassifier
						.classifyInstance(sparseInst);

				String predictString = thisClassAttribute
						.value((int) predictedValue)
						+ " ("
						+ predictedValue
						+ ")";
				...
				{
					String correctString = thisClassAttribute
							.value((int) correctValue)
							+ " ("
							+ correctValue
							+ ")";
				 ...
				 * double[] dist =
				 * ((Distribution)thisClassifier).distributionForInstance
				 * (sparseInst);
				 * 
	...
	public StringBuffer classify()
	{
		...
		if ((this.classString == null) || "".equals(this.classString))
		{
			return (new StringBuffer());
		}

		return this.classify(this.classString);
	...
	public StringBuffer classify(String classString)
	{
		...
		this.instances = new Instances("data set", this.attributeInfo, 100);

		// set which attribute is the class attribute
		this.instances.setClass(this.classAttribute);

		try
		{

			this.instances = TextClassifier.populateInstances(this.inputText,
					this.inputClasses, this.instances, this.classAttribute,
					this.textAttribute);
			result.append("DATA SET:\n" + this.instances + "\n");

			// make filtered SparseData
			this.filteredData = TextClassifier.filterText(this.instances);

			// create Set of modelWords
			this.modelWords = new HashSet();
			Enumeration enumx = this.filteredData.enumerateAttributes();
			while (enumx.hasMoreElements())
			{
				Attribute att = (Attribute) enumx.nextElement();
				String attName = att.name().toLowerCase();
				this.modelWords.add(attName);
			...
			this.evaluation = new Evaluation(this.filteredData);
			this.evaluation.evaluateModel(this.classifier, this.filteredData);

			result.append(TextClassifier.printClassifierAndEvaluation(
					this.classifier, this.evaluation) + "\n");

			// check instances
			int startIx = 0;
			result.append(TextClassifier.checkCases(this.filteredData,
					this.classifier, this.classAttribute, this.inputText,
					"not test", startIx)
					+ "\n");
	...
	public StringBuffer classifyNewCases(String[] tests)
	{
		...
		Instances testCases = new Instances(this.instances);
		testCases.setClass(this.classAttribute);
		...
		for (int i = 0; i < tests.length; i++)
		{
			...
			String[] splittedText = tests[i].split("["
					+ this.delimitersStringToWordVector + "]");
			// check if word is a model word
			for (String sWord : splittedText)
			{
				if (this.modelWords.contains(sWord))
				{
					gotModelWords++;
		...
		try
		{
			...
			testCases = TextClassifier.populateInstances(testsWithModelWords,
					tmpClassValues, testCases, this.classAttribute,
					this.textAttribute);

			// result.append("TEST CASES before filter:\n" + testCases + "\n");

			Instances filteredTests = TextClassifier.filterText(testCases);
			...
			// check
			//
			int startIx = this.instances.numInstances();
			result.append(TextClassifier.checkCases(filteredTests,
					this.classifier, this.classAttribute, tests, "newcase",
					startIx)
					+ "\n");
